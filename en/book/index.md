---
title: The Book
description: Out of Context, Second Edition is the main argument and practical spine.
kicker: Read
---

*Out of Context* is a second-edition theory and operating manual for leaders, operators, and serious readers trying to understand why modern work became noisy, brittle, performative, and hard to govern.

The release-candidate manuscript positions context collapse as the diagnosis and formation after context collapse as the work. It is not anti-remote, anti-AI, anti-youth, anti-purpose, or nostalgic for old hierarchy.

The main book now carries a complete 45-chapter spine across four parts: symptoms, diagnosis, reconstruction, and adult formation. The print route keeps the backmatter compact, while the companion library carries the longer appendix papers, workbook material, and source trail.

## Who This Book Is For

- CEOs, operators, and senior leaders who need a sharper diagnosis than "people are resistant to change."
- People, organization, and transformation leaders trying to rebuild authority, conflict, learning, and cadence without returning to command-and-control theater.
- Managers and individual contributors who feel the collapse daily: too many meetings, too many artifacts, too few real decisions, and too little formation.
- Serious readers of work, AI, institutions, and adulthood who want more than productivity advice.

## Start By Pain

- **Nothing gets decided:** read *Structural Collapse*, then *Healthy Hierarchies and Leadership*.
- **Everything is "fine":** read *Meaning Collapse*, *Decency Collapse*, and *Conflict Infrastructure*.
- **AI creates more work instead of better judgment:** read *Dancing with Intelligence* and *The Missing Rung*.
- **Juniors stop learning:** read *The Deskilling Disaster*, *Knowledge Collapse*, and *The Missing Rung*.
- **The business is illegible:** read *Value Creation Collapse* and *Operational Clarity*.

## What Changed In The Second Edition

- The Panta Rhei series is now a published seven-volume research trail rather than a promise.
- The AI argument has matured into an AI-era formation argument.
- The companion library carries the appendix papers, workbook material, and source compendium outside the printed spine.
- Evidence boundaries are stronger: public research, composite scenes, local observation, author models, practice heuristics, analogies, and speculative bridges are labeled separately.
- The five-framework operating system now gives readers a practical route from diagnosis to action.

## Start Here

- [Table of contents]({{ '/en/book/table-of-contents/' | relative_url }})
- [Sample pages and excerpts]({{ '/en/book/sample/' | relative_url }})
- [Buy and formats]({{ '/en/buy/' | relative_url }})
